...
export default function CapabilityDetailPage({ params }: { params: { capabilitySlug: string } }) {
  ...
  return <div className="page-container"><PageIntro eyebrow="能力详情 · 解释性导览" title={capability.name} description={capability.summary}><TrackedLink className="button button-primary" href="/projects" event={{ name: 'next_step_select', from: 'capability_detail', target: 'project' }}>找一张项目体验卡 <span aria-hidden="true">→</span></TrackedLink><Link className="button button-secondary" href="/capabilities">回到 {siteData.capabilities.length} 类能力</Link></PageIntro>
    ...
    <section className="detail-block"><SectionHeading eyebrow="下一步" title="把能力变成一个可说明的成果" /><div className="card-grid card-grid-3">{model.projects.map((project) => <article className="side-card" key={project.id}><Badge tone="amber">项目</Badge><h3 className="detail-card-heading project-heading"><TrackedLink href={`/projects/${project.slug}`} event={{ name: 'next_step_select', from: 'capability_detail', target: 'project' }}>{project.title}</TrackedLink></h3><p>{project.outputSummary}</p><ArrowLink href={`/projects/${project.slug}`}>打开体验卡</ArrowLink></article>)}</div></section>
    <section className="detail-block"><SectionHeading eyebrow="反向关系" title="这项能力还能进入哪些场景" /><div className="relation-list relation-list-grid">{model.scenarios.map((scenario) => <TrackedLink key={scenario.id} href={`/scenarios/${scenario.slug}`} event={{ name: 'next_step_select', from: 'capability_detail', target: 'scenario' }}>{scenario.name}<span>{scenario.taskSummary}</span></TrackedLink>)}</div></section>
