...
export default function ScenariosPage() {
  ...
  return <div className="page-container"><PageIntro eyebrow="能力迁移 · 场景不是岗位榜单" title="同一项能力，在不同问题里会遇到不同约束" description="从医疗健康出发，也可以看到 AI/数据、软件系统、电子物联、机器人/制造和环境/城市/公共技术。每张卡都同时说明共用能力与额外门槛。"><Link className="button button-primary" href="#scenario-list">浏览 {siteData.scenarios.length} 类场景 <span aria-hidden="true">↓</span></Link><Link className="button button-secondary" href="/capabilities">回到能力地图</Link></PageIntro>
    <section className="detail-block" id="scenario-list"><SectionHeading eyebrow="场景总览" title="不按行业热度排序，只按问题和能力关系来读" description="场景卡是学习导航，不是企业推荐、岗位保证或职业预测。" /><div className="card-grid card-grid-3">{siteData.scenarios.map((scenario, index) => <ScenarioCard key={scenario.id} scenario={scenario} index={index} capabilityLinks={scenario.sharedCapabilities.map((id) => capabilityMap.get(id)).filter(Boolean) as Array<{ id: string; slug: string; name: string }>} />)}</div></section>
