...
export default function MajorComparePage() {
  ...
  return <div className="page-container">
    <PageIntro eyebrow={`两个专业 · ${siteConfig.currentCohort} 级课程依据`} title="面对同一道健康工程问题，两边会先做什么？" description="先看共同的工程基础，再看两种视角怎样拆任务、交付结果并接在一起。这不是难度排名，也不替你作选择。"><Link className="button button-primary" href="#dual-lens">看一次真实问题拆解 <span aria-hidden="true">↓</span></Link><Link className="button button-secondary" href="/capabilities">再看 {siteData.capabilities.length} 类能力</Link></PageIntro>
    <section className="detail-block"><SectionHeading eyebrow="先看共同点" title="两个专业都要把问题变成可验证的工程任务" description="共同学习数学与自然科学、生命健康、编程与电子、信号系统、实验研究和工程责任；差别从任务侧重开始。" /><FoundationTable majors={siteData.majors} /></section>
    ...
    <section className="detail-block" id="dual-lens"><SectionHeading eyebrow="同一道题" title="分别做出什么，又在哪里交接？" description="从共同目标出发，再看两种视角的输入、输出和接口。" /><div className="dual-grid">{siteData.dualLensCases.map((item) => <DualLensCard key={item.id} item={item} majorMap={majorMap} />)}</div></section>
