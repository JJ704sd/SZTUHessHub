...
type CurriculumProps = { params: Promise<{ majorSlug: string; cohort: string }> };

export async function generateMetadata({ params }: CurriculumProps): Promise<Metadata> {
  const { majorSlug, cohort } = await params;
  const major = siteData.majors.find((item) => item.slug === majorSlug);
  return { title: `${major?.shortName ?? '专业'} · ${cohort} 级课程入口`, description: 'HseeHub 版本化培养方案导览', alternates: siteConfig.isProduction ? { canonical: `/majors/${majorSlug}/curriculum/${cohort}` } : undefined };
}

export default async function CurriculumPage({ params }: CurriculumProps) {
  const { majorSlug, cohort } = await params;
  const major = siteData.majors.find((item) => item.slug === majorSlug);
  if (!major || cohort !== siteConfig.currentCohort) notFound();
  ...
  return <div className="page-container"><PageIntro eyebrow={`${major.shortName} · 版本快照`} title={`${cohort} 级培养方案导览`} description={`${major.name}的课程、选修和实践入口。这里是解释性导览，不替代学校正式培养方案、教务通知或选课系统。`}><Link className="button button-secondary" href={`/majors/${major.slug}`}>回到专业介绍</Link><Link className="button button-secondary" href="/sources">查看来源版本</Link></PageIntro>
    ...
    <section className="detail-block"><SectionHeading eyebrow="课程 DNA" title="课程为什么会连接到这些能力" description="先看课程证据，再打开能力与任务关系；不要把课程名孤立地当作专业结论。" /><div className="card-grid card-grid-3">{major.courseEvidence.map((evidence) => <article className="side-card" key={evidence.course}><div className="tag-row">{evidence.capabilityIds.slice(0, 2).map((id) => { const capability = capabilityMap.get(id); return capability ? <Link className="badge badge-muted badge-link" key={id} href={`/capabilities/${capability.slug}`}>{capability.shortName}</Link> : null; })}</div><h3 className="detail-card-heading course-heading">{evidence.course}</h3><p>{evidence.detail}</p></article>)}</div></section>
