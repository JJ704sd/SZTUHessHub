...
function ResourceCard({ resource, primary, now }: { resource: ResourceLink; primary: boolean; now: Date }) {
  ...
  const action = internal ? <Link className={primary ? 'button button-primary' : 'button button-secondary'} href={resource.url}>{primary ? '开始 starter' : '打开站内入口'} <span aria-hidden="true">→</span></Link> : <a className="button button-secondary" href={resource.url} target="_blank" rel="noreferrer">打开外部资源 <span aria-hidden="true">↗</span></a>;
