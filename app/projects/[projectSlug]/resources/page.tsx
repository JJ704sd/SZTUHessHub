...
import { siteData, resourceManifests, type ResourceLink } from '@/lib/content';
import { siteConfig } from '@/lib/site-config';
import { canBePrimary, deriveResourceStatus, makeLegacyResourceManifest, primaryResourceConditions, resourceStatusLabel } from '@/lib/resources';
...
function ResourceLink({ resource, projectId, primary, now }: { resource: ResourceLink; projectId: string; primary: boolean; now: Date }) {
  ...
  const link = resource.url.startsWith('/') ? <TrackedLink className={primary ? 'button button-primary' : 'button button-secondary'} href={href} event={{ name: 'starter_begin', projectId, resourceId: resource.id }}>{primary ? '开始 starter' : '打开站内入口'} <span aria-hidden="true">→</span></TrackedLink> : <TrackedLink className="button button-secondary" href={href} target="_blank" rel="noreferrer" event={{ name: 'external_resource_open', projectId, resourceId: resource.id }}>打开外部资源 <span aria-hidden="true">↗</span></TrackedLink>;
  return <article className="resource-card"><div className="resource-card-head"><div><Badge tone={status === 'verified' ? 'teal' : status === 'unavailable' ? 'amber' : 'muted'}>{resourceStatusLabel(status)}</Badge><h3>{resource.title}</h3></div><span className="card-kicker">{resource.kind}</span></div><dl className="resource-facts"><div><dt>版本</dt><dd>{resource.version}</dd></div><div><dt>owner</dt><dd>{resource.ownerId}</dd></div><div><dt>机器</dt><dd>{resource.availability}{resource.automatedStatusCode ? ` · HTTP ${resource.automatedStatusCode}` : ''}{resource.lastAutomatedCheckAt ? ` · ${resource.lastAutomatedCheckAt}` : ''}</dd></div><div><dt>人工</dt><dd>{resource.reviewStatus}{resource.lastHumanWalkthroughAt ? ` · ${resource.lastHumanWalkthroughAt}` : ' · 未登记走通时间'}</dd></div><div><dt>freshness</dt><dd>{conditions.fresh ? 'fresh · 30 天内' : 'not fresh'}</dd></div><div><dt>许可</dt><dd>{resource.license}</dd></div></dl>{resource.failureReason ? <p className="resource-warning">{resource.failureReason}</p> : null}<div className="resource-actions">{link}{primary && status !== 'verified' ? <span className="resource-action-note">条件未齐全，不升级为主 CTA</span> : null}</div>{resource.internalFallbackPath && resource.internalFallbackPath !== href ? <p className="resource-fallback">替代入口：<Link href={resource.internalFallbackPath}>回到项目说明</Link></p> : null}<small className="resource-url">权威路径：{resource.url}</small></article>;
}
...
export default function ProjectResourcesPage({ params }: { params: { projectSlug: string } }) {
  ...
  const primaryReady = primary ? canBePrimary(primary, now) : false;
