...
type StarterProps = { params: Promise<{ projectSlug: string }> };

export async function generateMetadata({ params }: StarterProps): Promise<Metadata> {
  const { projectSlug } = await params;
  return { title: 'signal-feature-notebook · 10 分钟 starter', description: '无需账号、零安装的合成信号 starter。', alternates: siteConfig.isProduction ? { canonical: `/projects/${projectSlug}/starter` } : undefined };
}
...
export default async function SignalFeatureStarterPage({ params }: StarterProps) {
  const { projectSlug } = await params;
