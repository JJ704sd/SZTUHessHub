...
export type AnalyticsEvent =
  | { name: 'task_area_viewed'; surface: 'home'; sessionId: 'anonymous' }
  | { name: 'home_task_select'; taskId: 'compare' | 'capabilities' | 'project'; surface: 'task-grid' }
  | { name: 'dual_lens_open'; caseId: string; source: 'home' | 'compare' }
  | { name: 'capability_open'; capabilityId: string; source: 'home' | 'capabilities' | 'compare' }
...
const allowedEventFields: Record<AnalyticsEvent['name'], string[]> = {
  task_area_viewed: ['name', 'surface', 'sessionId'], home_task_select: ['name', 'taskId', 'surface'], dual_lens_open: ['name', 'caseId', 'source'], capability_open: ['name', 'capabilityId', 'source'], next_step_select: ['name', 'from', 'target'], project_open: ['name', 'projectId', 'source'], starter_begin: ['name', 'projectId', 'resourceId'], external_resource_open: ['name', 'projectId', 'resourceId'],
};
...
export function isPrivacySafeEvent(event: AnalyticsEvent) {
  ...
  return fields.every((field) => allowedEventFields[event.name]?.includes(field)) && !fields.some((field) => /url|query|text|email|ip|student|content/i.test(field));
}
