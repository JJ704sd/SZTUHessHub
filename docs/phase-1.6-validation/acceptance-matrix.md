...
| A04 | 阻塞 | `home-sections.tsx`、Phase 1.6 CSS；关键功能 13/13 通过；现有 33 个 visual golden 均产生可复现 diff，未替换候选 | 主代理 / 指定维护者待批准 golden | 未获非提交者/维护者批准；未知 visual diff 必须阻断 |
...
| A11 | 部分通过/阻塞 | `npm run check`、关键 Playwright 13/13、axe、E2E smoke、a11y smoke 通过；`test:visual` 33/33 被 golden diff 阻断 | 发布负责人待指定 | Lighthouse 工具/分数与生产 smoke 尚不可用；golden 仍待批准 |
