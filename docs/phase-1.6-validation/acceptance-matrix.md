...
| --- | --- | --- | --- | --- |
| A01 | 通过 | `codex/phase-1.6` HEAD=`e6df957`；`npm run check` 通过；对比记录在 `baseline.md` | 主代理 | 无；未在旧基线上重复 Phase 1.5 |
...
| A04 | 通过 | `home-sections.tsx`、Phase 1.6 CSS、33 张 visual candidate；仅任务入口保留明显阴影 | 主代理 / 指定维护者待批准 golden | golden 尚未获非提交者/维护者批准 |
...
| A08 | 通过 | evidence 聚合单测覆盖 unverified/degraded/unavailable/available/批准替代；`resources:probe` 10/10；筛选 URL、恢复、前进后退 E2E | 主代理 / 内容负责人 | `content/resource-health.json` 未在无批准下刷新；探测报告未直接改内容，符合门禁 |
...
| A11 | 部分通过/阻塞 | `npm run check`、Playwright 42/42、`test:visual` 33/33、axe、两个 smoke 通过 | 发布负责人待指定 | Lighthouse 工具/分数与生产 smoke 尚不可用；golden 仍待批准 |
...
| A14 | 阻塞 | `artifacts/perf-ci.json` 记录 3-run gate 未产出样本 | 性能/发布负责人待指定 | 环境未提供 Lighthouse；临时 npx 尝试未在超时内完成，不能宣称中位数达标 |
