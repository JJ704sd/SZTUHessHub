...
export function HomeTaskLauncher({ model }: { model: HomePageModel }) {
  return <section className="home-launch" data-home-module="task-entry">
    <div className="page-container home-launch-inner">
      ...
      <div className="task-grid" aria-label="学生任务入口">{model.tasks.map((task, index) => <TrackedLink className={index === 0 ? 'task-card task-card-primary' : 'task-card'} href={task.href} key={task.id} event={{ name: 'home_task_select', taskId: task.id === 'projects' ? 'project' : task.id as 'compare' | 'capabilities', surface: 'task-grid' }}><span className="task-icon" aria-hidden="true">{task.icon}</span><span><strong>{task.label}</strong><small>{task.summary}</small></span><span className="task-arrow" aria-hidden="true">→</span></TrackedLink>)}</div>
...
export function HomeMajorCompare({ model }: { model: HomePageModel }) {
  return <section className="home-section home-section-compare" data-home-module="quick-compare" id="home-compare"><div className="page-container"><SectionKicker number="01" label="双专业一屏对照" title="共同底座相同，工程侧重不同，也可以接在一起" action={{ href: '/majors/compare', label: '进入完整对照' }} />
...
export function HomeProjectPreviews({ model }: { model: HomePageModel }) {
  return <section className="home-section home-section-projects" data-home-module="projects" id="home-projects"><div className="page-container"><SectionKicker number="02" label="今天先试一个" title="先看时长、基础、产出和状态，再打开外部工具" action={{ href: '/projects', label: '比较全部体验卡' }} /><div className="home-project-grid">{model.projects.map((project) => <HomeProjectPreview key={project.id} project={project} />)}</div></div></section>;
}
