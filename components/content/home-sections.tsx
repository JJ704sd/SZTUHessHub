...
export function HomeTaskLaunchpad({ model }: { model: HomePageModel }) {
  ...
  return <section className={styles.launch} aria-labelledby="home-launch-title"><div className={`page-container ${styles.launchGrid}`}><div><p className="eyebrow">给健康工程学生的低风险实验桌</p><h1 id="home-launch-title">今天先碰一个小问题。</h1><p className={styles.lede}>不用先选专业，也不用先装环境。看懂差异，或花 10 分钟试一次。</p><nav className={styles.taskList} aria-label="开始探索">{model.homeActions.map((action) => { const displayLabel = action.isPrimary && !action.directStart ? action.statusLabel : action.label; return <Link data-home-task-entry="true" data-home-action-id={action.id} data-home-action-status={action.status} data-home-action-direct-start={action.directStart} data-home-action-fallback={action.fallbackHref} className={action.isPrimary ? styles.primaryTask : styles.secondaryTask} href={action.href} aria-label={`${displayLabel}${action.isPrimary && !action.directStart ? `，将先查看项目状态` : ''}`} key={action.id}><span><strong>{displayLabel}</strong><small>{action.summary}</small></span><span aria-hidden="true">→</span></Link>; })}</nav></div>{preview ? <figure className={styles.resultPreview}><img src={preview.src} alt={preview.alt} width="720" height="460" /><figcaption><strong>做完会留下：曲线与三行观察</strong><span>10 分钟 · 可本地保存 · 不含真实健康数据</span><small>确定性合成预览，不代表真实学生参与。</small></figcaption></figure> : null}</div></section>;
}
...
export function HomeDualMajorCase({ model }: { model: HomePageModel }) {
  ...
  return <section className={styles.section} aria-labelledby="home-dual-title"><div className="page-container"><div className={styles.heading}><div><p className="eyebrow">30 秒看懂专业侧重</p><h2 id="home-dual-title">同一道题，两边主要负责什么</h2><p>共同问题：{item.problem}</p></div><Link href={compareAction.href}>看完整分工与接口 →</Link></div><div className={styles.dualStrip}>{item.lenses.map((lens) => <article key={lens.majorId}><strong>{lens.label}</strong><p>{lens.role}</p></article>)}<p className={styles.shared}><strong>共同产物：</strong>{item.sharedArtifact}</p></div></div></section>;
}

export function HomeFeaturedProjects({ model }: { model: HomePageModel }) {
  const primaryAction = model.homeActions.find((action) => action.isPrimary);
  return <section className={styles.section} aria-labelledby="home-projects-title"><div className="page-container"><div className={styles.heading}><div><p className="eyebrow">挑一个能完成的小项目</p><h2 id="home-projects-title">三个任务，只保留影响开始的事实</h2></div><Link href="/projects?intent=quick-look">看全部小项目 →</Link></div><div className={styles.projectList}>{model.featuredProjects.map((project) => { const representative = project.id === primaryAction?.projectId; const external = getProjectResourceState(project); const actionHref = representative ? (primaryAction?.fallbackHref ?? project.primaryAction.href) : project.primaryAction.href; const actionLabel = representative ? (primaryAction?.fallbackLabel ?? project.primaryAction.label) : '先看怎么开始'; return <article data-home-project-entry="true" key={project.id}><div><h3>{project.title}</h3><p>{project.duration} · {project.prerequisites[0]} · 会留下 {project.expectedOutput}</p><span>内部起点：{representative ? `10 分钟 Starter（${primaryAction?.statusDetail ?? primaryAction?.statusLabel ?? '待复核'}）` : '只有开始说明'} · 外部资源：{external.label}</span></div><Link className={styles.rowAction} href={actionHref} aria-label={`${actionLabel}${representative && primaryAction && !primaryAction.directStart ? `，当前${primaryAction.statusLabel}` : ''}`}>{actionLabel} <span aria-hidden="true">→</span></Link></article>; })}</div></div></section>;
}
