...
export function HomeTaskLaunchpad({ model }: { model: HomePageModel }) {
  ...
  return <section className={styles.launch} aria-labelledby="home-launch-title"><div className={`page-container ${styles.launchGrid}`}><div><p className="eyebrow">给健康工程学生的低风险实验桌</p><h1 id="home-launch-title">今天先碰一个小问题。</h1><p className={styles.lede}>不用先选专业，也不用先装环境。看懂差异，或花 10 分钟试一次。</p><nav className={styles.taskList} aria-label="开始探索"><Link className={styles.secondaryTask} href="/majors/compare#dual-lens"><span><strong>看两个专业怎么分工</strong><small>用同一道题看两种工程视角</small></span><span aria-hidden="true">→</span></Link><Link className={styles.primaryTask} href={starterHref}><span><strong>做一次 10 分钟 Starter</strong><small>无账号、零安装、只用合成信号</small></span><span aria-hidden="true">→</span></Link><Link className={styles.secondaryTask} href="/pathways/explore"><span><strong>我还没想好</strong><small>用两条短任务做一周比较</small></span><span aria-hidden="true">→</span></Link></nav></div>{preview ? <figure className={styles.resultPreview}><img src={preview.src} alt={preview.alt} width="720" height="460" /><figcaption><strong>做完会留下：曲线与三行观察</strong><span>10 分钟 · 可本地保存 · 不含真实健康数据</span><small>确定性合成预览，不代表真实学生参与。</small></figcaption></figure> : null}</div></section>;
}
